...
loop do
  ...
  if op_code == '30'
    ...
    game_info_bytes = bytes[20..-23]
    game_settings = game_info_bytes[0..3].pack('C*').unpack('L<')[0]
    zero_flag = game_info_bytes[31]
    #game_info_string = bytes[20..-23].pack('C*').unpack('Z*')[0]
    name = get_string_segment(bytes[20..-1])
    encoded_segment_start_index = 22 + utf8_byte_count(name)
    decrypted = decode_string_part(bytes[encoded_segment_start_index..-1]).bytes
    game_settings = decrypted[0..3].pack('C*').unpack('L<')[0]
    #map_name = game_info_bytes[39..-1].pack('C*').unpack('Z*')[0]
    decrypted_string = decrypted.pack('C*')

    # Extrahiere den Karten-Namen ab dem 14. Byte
    map_name = decrypted_string[13..-1]

    # Finde das letzte Segment des Karten-Namens, das durch '\\' getrennt ist
    last_map_name_segment = map_name.split('\\').last
    last_map_name_segment = last_map_name_segment[0..(last_map_name_segment.index('.w3')+4)]

    # Wenn kein Segment gefunden wurde, bleibt der ursprüngliche Kartenname erhalten
    map_name = last_map_name_segment || map_name

    map_width = decrypted[5..-1].pack('C*').unpack('S<')[0]
    map_height = decrypted[7..-1].pack('C*').unpack('S<')[0]
  ...
  puts "UPTIME: #{system_tick_counts}"
  puts "GAME SETTINGS: #{game_settings}"
  puts "NAME: #{name}"
  puts "ZERO FLAG: #{zero_flag}"
